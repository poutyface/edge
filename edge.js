var SpatialFiltering, edgeDetect, getPixel, setPixel, toGrayscale;
...
edgeDetect = function(source, dest, width, height) {
  ...
  for (y = 0; (0 <= height ? y < height : y > height); (0 <= height ? y += 1 : y -= 1)) {
    for (x = 0; (0 <= width ? x < width : x > width); (0 <= width ? x += 1 : x -= 1)) {
      _ref = getPixel(source, x, y), r = _ref[0], g = _ref[1], b = _ref[2];
  ...
  for (y = 0; (0 <= height ? y < height : y > height); (0 <= height ? y += 1 : y -= 1)) {
    _results.push((function() {
      ...
      for (x = 0; (0 <= width ? x < width : x > width); (0 <= width ? x += 1 : x -= 1)) {
        ...
        _results.push(setPixel(dest, x, y, I, I, I, 255));
      }
      return _results;
    })());
  }
